...
const DashboardPage = () => {
  ...
  const dashboardFilters: DashboardFilterParams = useMemo(
    () => ({ pais: filters.pais, gestor: filters.gestor, gerente: filters.gerente, zona: filters.zona, pd: filters.pd, campania: filters.campania }),
    [filters]
  );
  ...
  useEffect(() => {
    ...
    if (
      !isSame(sanitized.pais, filters.pais) || !isSame(sanitized.zona, filters.zona) ||
      !isSame(sanitized.gestor, filters.gestor) || !isSame(sanitized.gerente, filters.gerente) ||
      !isSame(sanitized.pd, filters.pd) || !isSame(sanitized.campania, filters.campania)
  ...
  if (!hasFiltersApplied && (dashboard.kpis?.totalCuentas ?? 0) === 0) {
    return <Box sx={{ p: 4, textAlign: 'center' }}><Typography sx={{ fontSize: 16, fontWeight: 600 }}>No hay datos disponibles para tu alcance actual.</Typography><Typography sx={{ fontSize: 13, color: 'text.secondary', mt: 0.5 }}>No tienes cuentas asignadas dentro de tu alcance de acceso.</Typography></Box>;
  }
    ...
    <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(12, minmax(0, 1fr))', gap: 2, alignItems: 'stretch', width: '100%' }}>
      ...
      <Box sx={{ gridColumn: '1 / -1' }}>
        <Box sx={{ display: 'grid', gap: 2 }}>
          ...
          <Box sx={{ '& > .MuiPaper-root:first-of-type': { display: 'none' } }}>
