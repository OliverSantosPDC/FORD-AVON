...
const GestionMasivaUsuarios = () => {
  ...
  const onDescargarReporte = () => { if (!result) return; const headers = ['HOJA','FILA','ACCION','EMAIL','NOMBRE','APELLIDO','ROL','ESTADO','CONTRASEÑA_TEMPORAL','MENSAJE']; const rows = result.resultados.map((r) => [r.hoja,r.fila,r.accion,r.email,r.nombre ?? '',r.apellido ?? '',r.rol ?? '',r.resultado,r.password ?? '',r.mensaje]); exportRowsToExcel('resultado_usuarios.xlsx','Resultado',headers,rows); };
