...
const GestionMasivaUsuarios = () => {
  ...
  const onValidar = async () => { if (!file) return; setBusy(true); setError(null); setResult(null); try { setPreview(await validarImportacion(file)); } catch (err) { setError(err instanceof Error ? err.message : 'No se pudo validar el archivo.'); } finally { setBusy(false); } };
  const onAplicar = async (soloValidas: boolean) => { if (!file) return; setBusy(true); setError(null); try { const res = await aplicarImportacion(file, soloValidas); setResult(res); setPreview(null); } catch (err) { setError(err instanceof Error ? err.message : 'No se pudo procesar el archivo.'); } finally { setBusy(false); } };
  const onDescargarReporte = () => { if (!result) return; const headers = ['FILA','ACCION','EMAIL','NOMBRE','APELLIDO','ROL','ESTADO','CONTRASEÑA_TEMPORAL','MENSAJE']; const rows = result.resultados.map((r) => [r.fila,r.accion,r.email,r.nombre ?? '',r.apellido ?? '',r.rol ?? '',r.resultado,r.password ?? '',r.mensaje]); exportRowsToExcel('resultado_usuarios.xlsx','Resultado',headers,rows); };
